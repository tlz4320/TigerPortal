...
total_used_seq$cds_start <- unlist(lapply(1 : nrow(total_used_seq), function(i){
  str_locate(total_used_seq$seq[i], total_used_seq$cds[i])[1,1]
  
}))

aa_change_stat <- lapply(names(noAA_change_seq), function(name){
  tmp <- noAA_change_seq[[name]]
  res <- lapply(names(tmp), function(x){

    seq_table <- tmp[[x]]
    if(nrow(seq_table) == 0){
      return(data.frame(diff = 0, Pct = 0))
    }
    seq_table <- seq_table[order(seq_table$Pct,decreasing = T),]
    dis <- c(total_used_seq_tissue$mutPos - total_used_seq_tissue$cutSite)[total_used_seq_tissue$result == name]
    cutsite <- total_used_seq_tissue$cutSite[total_used_seq_tissue$result == name]
    seq_table$dis <- ifelse(dis <= 0, dis - 1, dis)
    mutpos <- total_used_seq_tissue$mutPos[total_used_seq_tissue$result == name]
    
    strand <- total_used_seq_tissue$strand[total_used_seq_tissue$result == name]
    #insert NT will be cap 
    mutseq <- str_to_lower(total_used_seq_tissue$mutSeq[total_used_seq_tissue$result == name])
    sg <- str_to_lower(total_used_seq_tissue$sg[total_used_seq_tissue$result == name])
    wtseq <-  unlist(strsplit(total_used_seq_tissue$seq[total_used_seq_tissue$result == name], "*"))
    cds_start <- total_used_seq$cds_start[total_used_seq$result == name]
    seq_table$diff <- unlist(lapply(1 : nrow(seq_table), function(i){
      ...
      mutseqs <- mutseqs[cds_start : length(mutseqs)]
      wtseq <- wtseq[cds_start : length(wtseq)]
      mutp <- seqinr::translate(seqinr::s2c(paste0(mutseqs, collapse = "")), ambiguous = T)
      wtp <- seqinr::translate(seqinr::s2c(paste0(wtseq, collapse = "")), ambiguous = T)
      return(sum(mutp != wtp))
    }))
    
    res <- lapply(split(seq_table$Pct, seq_table$diff), sum)
    data.frame(diff = names(res), Pct = unlist(res))
  })
  res <- data.frame(do.call(rbind, res))
  res <- lapply(split(res, res$diff), function(x){
    pct <- x$Pct
    data.frame(diff = unique(x$diff), Pct = mean(pct), se = plotrix::std.error(pct))
  })
  res <- data.frame(do.call(rbind, res))
  res$id <- name
  res
...
aa_change_stat <- data.frame(do.call(rbind, aa_change_stat))
aa_change_stat$se[is.na(aa_change_stat$se)] <- 0
...
restore_stat_sel <- restore_stat[sel_for_detail_plot$id,]
reframe_pct_sel <- reframe_pct[reframe_pct$id %in% sel_for_detail_plot$id,]
...
# insert_mat_sel_append <- matrix(0, nrow = nrow(insert_mat_sel) * 3, ncol = (4 + 5 + 3))
# colnames(insert_mat_sel_append) <- c(colnames(insert_mat_sel), "-2AA", "-1AA", "0AA", 
#                                      "1AA", "2AA", "0Change", "1Change", "2Change")
# for(i in 1 : nrow(insert_mat_sel)){
#   insert_mat_sel_append[i * 3,1 : 4] <- insert_mat_sel_append[i * 3,1 : 4] / 
#     sum(insert_mat_sel_append[i * 3,1 : 4]) * 
#     insert_mat_sel_append[i * 3 - 2, 7]
# }
...
aa_change_sel <- aa_change_stat[aa_change_stat$id %in% sel_for_detail_plot$id,]
aa_change_sel <- lapply(split(aa_change_sel, aa_change_sel$id), function(x){
  x$Pct <- x$Pct / sum(x$Pct) * 100
  x[1,]
})
aa_change_sel <- data.frame(do.call(rbind, aa_change_sel))
rownames(aa_change_sel) <- aa_change_sel$id
aa_change_sel <- aa_change_sel[unique(edit_mat_sel$id),]
aa_change_sel$id <- factor(aa_change_sel$id, levels = aa_change_sel$id)

aa_change_append <- data.frame(do.call(rbind, lapply(split(aa_change_sel, aa_change_sel$id), function(x){
  data.frame(diff = "0", Pct = c(x$Pct, 0, 0), se = c(x$se, 0, 0), id = x$id)
})))

right_anno <- rowAnnotation("AA Ident Change" = anno_barplot(aa_change_append$Pct, 
                                                             bar_width = 0.5, 
                                                             width = unit(2, "cm"), 
                                                            ylim = c(0, 105)))
# right_anno <- rowAnnotation(InsertNT = anno_barplot(insert_mat_sel_append, 
#                                                     gp = gpar(fill = col, lwd = 0.1), 
#                                                     bar_width = 0.5, 
#                                                     width = unit(2, "cm")), 
...
#                             "Ins1 in Indel%" = anno_barplot(x = ins1_pct_sel_append$indel, 
#                                                             gp = gpar(lwd = 0.1, fill = "#BBBBBB"),width = unit(2, "cm"), bar_width = 0.5, ylim = c(0, 42)))

pdf("~/Nutstore Files/Tobin/Merged1NT/Ins1_edit_between(+-1)_result_v5.pdf", width = 10, height = 6)
...
for(i in 1:length(unique(lens))) {
  ...
  #   
  # })
  decorate_annotation("AA Ident Change", slice = i, {
    od = rev(od[[i]])
    grid.segments(aa_change_append$Pct[od] - aa_change_append$se[od],seq_along(od), 
                  aa_change_append$Pct[od] + aa_change_append$se[od],seq_along(od), default.units = "native")
    grid.segments(aa_change_append$Pct[od] - aa_change_append$se[od], 
                  seq_along(od) - 0.1, 
                  aa_change_append$Pct[od] - aa_change_append$se[od], seq_along(od) + 0.1, default.units = "native")
    grid.segments(aa_change_append$Pct[od] + aa_change_append$se[od], 
                  seq_along(od) - 0.1, 
                  aa_change_append$Pct[od] + aa_change_append$se[od], seq_along(od) + 0.1, default.units = "native")
