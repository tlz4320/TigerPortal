...
plot_data_ins <- hist(total_used_seq_tissue$inframePct[total_used_seq_tissue$mutType == "i"], breaks = seq(from=0, to=93, by=3))
plot_data_ins <- data.frame(inframePct = plot_data_ins$breaks[-1], 
                        counts = plot_data_ins$counts)
plot_data_ins$color <- "#FAB0B0"
plot_data_ins$color[plot_data_ins$inframePct > 30] <-"#FA1E1E" 
#plot_data_ins$color[plot_data_ins$inframePct > 50] <-"#FA1E1E" 
plot_data_ins$x <- plot_data_ins$inframePct - 1.5
anno_info <- lapply(c(30, 50, 60, 90), function(pct){
  sum(plot_data_ins$counts[plot_data_ins$x > pct])
})
anno_info <- data.frame(pct = c(30, 50, 60, 90), counts = unlist(anno_info))
anno_info <- paste(c(paste0("Total Counts:", sum(plot_data_ins$counts)), paste("Counts of >", anno_info$pct, ":", anno_info$counts, sep="")), collapse = "\n")
pdf("~/Nutstore Files/Tobin/Previous/total_inframe_hist_ins_mut.pdf", width = 6, height = 4)
ggplot(plot_data_ins) + geom_bar(aes(x = x, y = counts), 
                             color = "black",
                             stat="identity", fill = plot_data_ins$color) + 
  geom_text(x = 80, y = 3, label = anno_info, data = data.frame(a=1)) + 
  xlab("Inframe Frequency") + ylab("Counts") + 
  scale_y_continuous(breaks = 1 : 10) + 
  scale_x_continuous(breaks = c(0, 15, 30, 45, 60, 75, 90)) + 
  theme_bw() + 
...
plot_data_del <- hist(total_used_seq_tissue$inframePct[total_used_seq_tissue$mutType == "d"], breaks = seq(from=0, to=93, by=3))
plot_data_del <- data.frame(inframePct = plot_data_del$breaks[-1], 
                            counts = plot_data_del$counts)
plot_data_del$color <- "#FAB0B0"
plot_data_del$color[plot_data_del$inframePct > 30] <-"#FA1E1E" 
#plot_data_del$color[plot_data_del$inframePct > 50] <-"#FA1E1E" 
plot_data_del$x <- plot_data_del$inframePct - 1.5
anno_info <- lapply(c(30, 50, 60, 90), function(pct){
  sum(plot_data_del$counts[plot_data_del$x > pct])
})
anno_info <- data.frame(pct = c(30, 50, 60, 90), counts = unlist(anno_info))
anno_info <- paste(c(paste0("Total Counts:", sum(plot_data_del$counts)), paste("Counts of >", anno_info$pct, ":", anno_info$counts, sep="")), collapse = "\n")
pdf("~/Nutstore Files/Tobin/Previous/total_inframe_hist_del_mut.pdf", width = 6, height = 4)
ggplot(plot_data_del) + geom_bar(aes(x = x, y = counts), 
                                 color = "black",
                                 stat="identity", fill = plot_data_del$color) + 
  geom_text(x = 80, y = 6, label = anno_info, data = data.frame(a=1)) + 
  xlab("Inframe Frequency") + ylab("Counts") + 
  scale_y_continuous(breaks = 1 : 10) + 
  scale_x_continuous(breaks = c(0, 15, 30, 45, 60, 75, 90)) + 
  theme_bw() + 
...
# ggplot(plot_data) + 
#   geom_bar(aes(x = pct2, y = id, fill = `Number of AAChange`), 
