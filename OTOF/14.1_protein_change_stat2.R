...
load("~/Nutstore Files/Tobin/Previous/AA_stat.rda")
...
wt_0aa <- lapply(names(noAA_change_seq), function(id){
  ...
  wt_pct <- unlist(lapply(x, function(y){
    insertpos <- unlist(lapply(y[,2], function(x){
...
openxlsx::write.xlsx(wt_0aa, file="~/Nutstore Files/Tobin/Previous/wt_of_0aa_in_Ins1.xlsx", rowNames=F, colNames=T)
