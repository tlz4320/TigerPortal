...
wt_0aa <- openxlsx::read.xlsx("~/Nutstore Files/Tobin/Previous/wt_of_0aa_in_Ins1.xlsx")
wt_0aa <- wt_0aa[!wt_0aa$id %in% c("m12-tissue", "22-051", "37-051", "65-05", "33-051"),]
outside_sel <- abs(rowSums(dis_mat))
###这里是为了画存在WT修复的部分，所以要把不在我这个list里面的去掉
...
pdf("~/Nutstore Files/Tobin/Previous/Ins1_edit_distance_add_WT_0aa_v3.pdf", width = 11, height = 3)
library(gridtext)
ht <- Heatmap(plot_mat, 
              name = "EditDis",
