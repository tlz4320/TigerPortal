...
ins1_pct_table_cell <- lapply(unique(seqs_table$id), function(id){
  ins1_table <- seqs_table[seqs_table$id == id,]
  wt_seq <- wt_seqs[[id]]
  tmp <- getStat2(ins1_table, T)
  tmp$id <- id
...
indelphi_res_ins1 <- lapply(indelphi_118_res, function(x){
...
for(file in total_used_seq_tissue$id){
  ...
  forecast_118_res_ins1[[file]] <- tmp2
}
...
for(name in names(ins1_pct_table)){
  ...
  tmp <- indelphi_res_ins1[[name]]

  indelphi_diff <- sum(tmp$Pct[tmp$NT == leftnt]) / sum(tmp$Pct)
  
  tmp <- forecast_118_res_ins1[[name]]
...
pdf("~/Nutstore Files/Tobin/Previous/ins1_link_plot_add_predict.pdf", width = 6, height = 4)
...
pdf("~/Nutstore Files/Tobin/Previous/ins1_link_plot_rmForecast.pdf", width = 5.3, height = 4)
