...
insert_mat_recolor <- lapply(rownames(insert_mat), function(x){
  pct <- insert_mat[x, ]
  sg <- total_sample_info$sg[total_sample_info$tissueid == x]
  
  leftnt <- unlist(strsplit(sg, "*"))
  if(total_sample_info$strand[total_sample_info$tissueid == x] == "-"){
    leftnt <- str_to_lower(as.character(Biostrings::reverseComplement(
      Biostrings::DNAString(leftnt[length(leftnt) - 3]))))
  } else {
    leftnt <- str_to_lower(leftnt[length(leftnt) - 3])
  }
  
  pct_leftnt <- pct[leftnt]
  pct[leftnt] <- 0
  return(c(pct_leftnt, unlist(pct)))
})
insert_mat_recolor <- data.frame(do.call(rbind, insert_mat_recolor))
colnames(insert_mat_recolor) <- c("Left", "a", "t", "c", "g")
cols = c("#FF4500", "#2F89FC","#30E3CA","#66CD00", "#98ABEF")
names(cols) <- c("Left", "a", "t", "c", "g")
rownames(insert_mat_recolor) <- rownames(insert_mat)
load("~/Nutstore Files/Tobin/Previous/inframe_spec_result_cell_tissue.rda")
inframe_spec <- reshape2::dcast(inframe_result_processed, id ~ aa, value.var = "pct")
...
right_anno <- rowAnnotation(InsertNT = anno_barplot(insert_mat_recolor, 
                                                    gp = gpar(fill = cols, lwd = 0.1), 
                                                    bar_width = 0.5, 
                                                    width = unit(2, "cm")), 
                            emp = anno_empty(width = unit(2, "mm"), border = F),
                            "Inframe Spec" = anno_barplot(x = inframe_spec, width = unit(2, "cm"), 
...
# save.image("~/data/project/ear_project/gene_therapy_ll/Previews/4.3.3_data.rda")

pdf("~/Nutstore Files/Tobin/Previous/Ins1_edit_distance_heatmap_total_v6_recolor.pdf", width = 9.5, height = 18)
...
ht <- Heatmap(plot_mat, 
              ...
              cell_fun = function(j, i, x, y, w, h, fill) {
                ...
                if(j == 7){
                  ...
                  names(col) <- c("A", 'T','C','G')
                  grid.text(nt, x, y, 
                            gp = gpar(fontsize = 20, col = "#FF4500"))
                }else{
                  grid.text(nt, x, y, 
                            gp = gpar(fontsize = 18))
                }
