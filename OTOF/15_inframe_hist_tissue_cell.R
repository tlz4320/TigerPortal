...
tissue_inframe <- inframe_result_processed
tissue_inframe <- tissue_inframe[tissue_inframe$id %in% del_sg,]
tissue_inframe <- unlist(lapply(split(tissue_inframe$pct, as.character(tissue_inframe$id)), sum))
plot_data <- hist(tissue_inframe, breaks = seq(from=0, to=105, by=5))
...
p1 <- ggplot(plot_data) + geom_bar(aes(x = x, y = counts),
  ...
  xlab("inframe/Inframe Frequency") + ylab("Counts") +
...
cell_inframe <- cell_inframe_result_processed
cell_inframe <- cell_inframe[cell_inframe$id %in% del_sg,]
cell_inframe <- unlist(lapply(split(cell_inframe$pct, as.character(cell_inframe$id)), sum))
plot_data <- hist(cell_inframe, breaks = seq(from=0, to=105, by=5))
plot_data <- data.frame(inframePct = plot_data$breaks[-1], 
                        counts = plot_data$counts)
max(plot_data$counts)
plot_data$color <- "#B46F2877"
...
pdf("~/Nutstore Files/Tobin/Previous/inframe_histgram_delMut_v1.pdf", width = 6, height = 6)
...
tissue_inframe <- inframe_result_processed
tissue_inframe <- tissue_inframe[tissue_inframe$id %in% ins_sg$result,]
tissue_inframe <- unlist(lapply(split(tissue_inframe$pct, as.character(tissue_inframe$id)), sum))
plot_data <- hist(tissue_inframe, breaks = seq(from=0, to=105, by=5))
plot_data <- data.frame(inframePct = plot_data$breaks[-1], 
                        counts = plot_data$counts)
...
cell_inframe <- cell_inframe_result_processed
cell_inframe <- cell_inframe[cell_inframe$id %in% ins_sg$result,]
cell_inframe <- unlist(lapply(split(cell_inframe$pct, as.character(cell_inframe$id)), sum))
plot_data <- hist(cell_inframe, breaks = seq(from=0, to=105, by=5))
plot_data <- data.frame(inframePct = plot_data$breaks[-1], 
                        counts = plot_data$counts)
...
p2 <- ggplot(plot_data) + geom_bar(aes(x = x, y = counts),
                                   ...
                                   stat="identity", fill = plot_data$color) +
  geom_text(x = 10, y = -5, label = anno_info, data = data.frame(a=1)) +
  scale_y_reverse(expand = c(0,0), breaks = 1 : 8, limits = c(8, 0)) +
  scale_x_continuous(breaks = c(0, 15, 30, 45, 60, 75, 90, 100), position = "top", limits = c(0, 100)) +
  ylab("Counts") + xlab("") + 
  theme_bw() + 
  theme(axis.text.x = element_blank(), panel.grid = element_blank(),
        plot.margin =  unit(c(-1,0,1,0.2), "cm"))
pdf("~/Nutstore Files/Tobin/Previous/inframe_histgram_insMut_v1.pdf", width = 6, height = 6)
ggpubr::ggarrange(p1, p2, ncol = 1, common.legend = T)
dev.off()
