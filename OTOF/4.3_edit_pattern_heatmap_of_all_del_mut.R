...
total_sample_info <- split_name <- data.frame(
  ...
  mutation = c(total_sample_info$GRNAa, paste0("w-OTOF-1236dC-m", 12 : 15)))
...
insert_mat_recolor <- lapply(rownames(insert_mat), function(x){
  ...
  leftnt <- str_to_lower(leftnt[length(leftnt) - 2])
...
right_anno <- rowAnnotation(InsertNT = anno_barplot(insert_mat_recolor, 
                            ...
                            "Indel%" = anno_barplot(x = indel_stat$indel, width = unit(2, "cm"), 
                                                    ...
                                                    bar_width = 0.5, ylim = ceiling(c(0, max(indel_stat$indel + 1)))), 
                            emp2 = anno_empty(width = unit(2, "mm"), border = F),
                            "Ins1 in Indel%" = anno_barplot(x = ins1_pct$indel, 
                                                            gp = gpar(lwd = 0.1, fill = "#BBBBBB"),width = unit(2, "cm"), bar_width = 0.5, ylim = c(0, 100)))


pdf("~/Nutstore Files/Tobin/Previous/Ins1_edit_distance_heatmap_total_v4.pdf", width = 9, height = 18)
...
for(i in 1:length(unique(lens))) {
  ...
  decorate_annotation("Ins1 in Indel%", slice = i, {
    od = rev(od[[i]])
    #pushViewport(viewport(xscale = c(0.5, length(od) + 0.5), yscale = c(0, 1)))
