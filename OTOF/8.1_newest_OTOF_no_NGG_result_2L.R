...
samples <- list.files(pattern = "CRISPResso_on_2L")[seq(1, 6, 2)]
otof_result <- list()
...
ins_result_mean <- lapply(split(ins_result, paste0(ins_result$Aligned_Sequence, ins_result$Reference_Sequence)), function(x){
...
openxlsx::write.xlsx(ins_result_mean, file="~/Nutstore Files/Tobin/Merged1NT/OTOF_2L_ins_frame_restore_total.xlsx", colNames=T, rowNames=F)
ins_result_mean <- ins_result_mean[1:3,]
# ins_result_mean$Reference_Sequence[4] <- "CTCCCCGAGGGCGTGCCCCCGAA-CGGCAGTGGGCACGGT"
seq_matrix <- matrix("", nrow = nrow(ins_result_mean), ncol = 40)
for(i in 1 : nrow(ins_result_mean)){
  seq <- unlist(strsplit(ins_result_mean$Aligned_Sequence[i], "*"))
  seq_matrix[i,] <- seq
}
insert_matrix <- matrix(0, nrow = nrow(ins_result_mean), ncol = 40)
...
col_stat <-  lapply(otof_result_indel, function(x){
  tmp <- x[paste0(x[,1], x[,2]) %in% paste0(ins_result_mean$Aligned_Sequence,
                                            ins_result_mean$Reference_Sequence),]
  tmp <- tmp[order(tmp[,1]),]
  tmp2 <- ins_result_mean[paste0(ins_result_mean$Aligned_Sequence,
                                 ins_result_mean$Reference_Sequence) %in% paste0(tmp[,1], tmp[,2]),]
  tmp[,2] <- tmp2[,2]
...
openxlsx::write.xlsx(na.omit(col_stat_data),
                     file= "~/Nutstore Files/Tobin/Merged1NT/OTOF_2L_ins1_pct_in_indel.xlsx",
                     rowNames=F)
...
openxlsx::write.xlsx(tmp,
                     file= "~/Nutstore Files/Tobin/Merged1NT/OTOF_2L_ins1_in_reframe.xlsx",
                     rowNames=F)
...
ht <- Heatmap(fake_mat, rect_gp = gpar(col = "gray"), col = c("white", "white"), 
              ...
              cell_fun = function(j, i, x, y, w, h, fill) {
                ...
                if((i != 3 & j %in% c(25 : 27)) | (i == 3 & j %in% c(24 : 26))){
...
pdf("~/Nutstore Files/Tobin/Merged1NT/OTOF_sg1_no_ngg_2L_ins_top3.pdf", 
    width = 8, height = 1.3)
draw(ht)
...
del_result_mean <- lapply(split(del_result, paste0(del_result$Aligned_Sequence, del_result$Reference_Sequence)), function(x){
  x$se <- plotrix::std.error(x$Pct)
  x$Pct <- mean(x$Pct)
  x[1,]
})
...
openxlsx::write.xlsx(del_result_mean, file="~/Nutstore Files/Tobin/Merged1NT/OTOF_2L_del_frame_restore_total.xlsx", colNames=T, rowNames=F)

del_result_mean <- del_result_mean[1:3,]
seq_matrix <- matrix("", nrow = nrow(del_result_mean), ncol = 40)
for(i in 1 : nrow(del_result_mean)){
  seq <- unlist(strsplit(del_result_mean$Aligned_Sequence[i], "*"))
  seq_matrix[i,] <- seq
}
delete_matrix <- matrix(0, nrow = nrow(del_result_mean), ncol = 40)
...
col_stat <-  lapply(otof_result_indel, function(x){
  tmp <- x[paste0(x[,1], x[,2]) %in% paste0(del_result_mean$Aligned_Sequence, del_result_mean$Reference_Sequence),]
  # tmp$Pct <- tmp[,8] / sum(tmp[,8]) * 100
  res <- rep(0, 40)
  for(i in 1 : nrow(tmp)){
...
openxlsx::write.xlsx(na.omit(col_stat)[,c(1,3,2)],
                     file= "~/Nutstore Files/Tobin/Merged1NT/OTOF_2L_del2_pct_in_indel.xlsx", rowNames=F)
tmp <- del_result_mean[,c("Pct", "se")]
openxlsx::write.xlsx(tmp,
                     file= "~/Nutstore Files/Tobin/Merged1NT/OTOF_2L_del2_pct_in_reframe.xlsx", rowNames=F)
...
pdf("~/Nutstore Files/Tobin/Merged1NT/OTOF_sg1_no_ngg_2L_del_top3.pdf", 
    width = 8, height = 1.3)
draw(ht)
decorate_annotation("pct",slice = 1, {
  od = 1 : 40
...
otof_indel_circle2 <- otof_indel_circle2[order(otof_indel_circle2$Pct, decreasing = T),]
...
openxlsx::write.xlsx(stat_data, file="~/Nutstore Files/Tobin/Merged1NT/OTOF_2L_frame_restore_stat.xlsx")
