...
right_anno <- rowAnnotation(InsertNT = anno_barplot(insert_mat, gp = gpar(fill = c("#2F89FC","#30E3CA","#66CD00", "#98ABEF"), lwd = 0.1), 
                                                    bar_width = 0.5, 
                                                    width = unit(2, "cm")), 
                            ...
                            "Del1 in Indel%" = anno_barplot(x = del1_pct$indel, 
                                                            gp = gpar(lwd = 0.1, fill = "#BBBBBB"),width = unit(2, "cm"), bar_width = 0.5, ylim = c(0, 100)))


pdf("~/Nutstore Files/Tobin/Previous/Del1_edit_distance_heatmap_total_v3.pdf", width = 9, height = 5)
ht <- Heatmap(plot_mat, 
              name = "EditDis",
...
for(i in 1:length(unique(lens))) {
  ...
  decorate_annotation("Del1 in Indel%", slice = i, {
    od = rev(od[[i]])
    #pushViewport(viewport(xscale = c(0.5, length(od) + 0.5), yscale = c(0, 1)))
    grid.segments(del1_pct$indel[od] - del1_pct$se[od],seq_along(od), del1_pct$indel[od] + del1_pct$se[od],seq_along(od), default.units = "native")
    grid.segments(del1_pct$indel[od] - del1_pct$se[od], seq_along(od) - 0.1, del1_pct$indel[od] - del1_pct$se[od], seq_along(od) + 0.1, default.units = "native")
    grid.segments(del1_pct$indel[od] + del1_pct$se[od], seq_along(od) - 0.1, del1_pct$indel[od] + del1_pct$se[od], seq_along(od) + 0.1, default.units = "native")
    # grid.function(function(y) {list(y=y, x=0.2)},
    #               range="y", gp=gpar(lty=2) )
    #grid.yaxis()
