...
samples <- list.files(pattern = "CRISPResso_on_7R")[seq(1, 6, 2)]
otof_result <- list()
...
openxlsx::write.xlsx(ins_result_mean, file="~/Nutstore Files/Tobin/Merged1NT/OTOF_7R_ins_frame_restore_total.xlsx", colNames=T, rowNames=F)
...
col_stat <-  lapply(otof_result_indel, function(x){
  tmp <- x[paste0(x[,1], x[,2]) %in% paste0(ins_result_mean$Aligned_Sequence,ins_result_mean$Reference_Sequence),]
  tmp <- tmp[order(tmp[,1]),]
...
openxlsx::write.xlsx(na.omit(col_stat_data),
                     file= "~/Nutstore Files/Tobin/Merged1NT/OTOF_7R_ins1_pct_in_indel.xlsx",
                     rowNames=F)
...
openxlsx::write.xlsx(tmp,
                     file= "~/Nutstore Files/Tobin/Merged1NT/OTOF_7R_ins1_in_reframe.xlsx",
                     rowNames=F)
...
pdf("~/Nutstore Files/Tobin/Merged1NT/OTOF_sg1_no_ngg_7R_ins_top3.pdf", 
    width = 8, height = 1.3)
draw(ht)
...
openxlsx::write.xlsx(del_result_mean, file="~/Nutstore Files/Tobin/Merged1NT/OTOF_7R_del_frame_restore_total.xlsx", colNames=T, rowNames=F)
...
openxlsx::write.xlsx(na.omit(col_stat)[,c(1,3,2)],
                     file= "~/Nutstore Files/Tobin/Merged1NT/OTOF_7R_del2_pct_in_indel.xlsx", rowNames=F)
tmp <- del_result_mean[,c("Pct", "se")]
openxlsx::write.xlsx(tmp,
                     file= "~/Nutstore Files/Tobin/Merged1NT/OTOF_7R_del2_pct_in_reframe.xlsx", rowNames=F)
...
pdf("~/Nutstore Files/Tobin/Merged1NT/OTOF_sg1_no_ngg_7R_del_top3.pdf", 
    width = 8, height = 1.3)
draw(ht)
decorate_annotation("pct",slice = 1, {
  od = 1 : 40
...
openxlsx::write.xlsx(stat_data, file="~/Nutstore Files/Tobin/Merged1NT/OTOF_7R_frame_restore_stat.xlsx")
