...
plot_data <- hist(del1_info$tissue0AA, breaks = seq(from=0, to=105, by=5))
...
for(name in c("cell0AA", "forecast0AA", "indelphi0AA")){
...
total_plot_data$type <- str_remove(total_plot_data$type, "0AA")
...
p2 <- ggplot(total_plot_data) + geom_bar(aes(y = x, x = counts, fill = type),
  ...
  theme(plot.margin =  unit(c(0,1,0,0), "cm"), 
        ...
        panel.grid = element_blank())
pdf("~/Nutstore Files/Tobin/Previous/0aa_inframe_histgram_delMut_addAll_v2.pdf", width = 5, height = 5)
...
plot_data <- hist(ins1_info$tissue0AA, breaks = seq(from=0, to=105, by=5))
plot_data <- data.frame(inframePct = plot_data$breaks[-1], 
                        counts = plot_data$counts)
max(plot_data$counts)
...
p2 <- ggplot(total_plot_data) + geom_bar(aes(y = x, x = counts, fill = type),
  ...
  theme(plot.margin =  unit(c(0,1,0,0), "cm"), 
        ...
        axis.text.y = element_text(hjust = 0.5),
        panel.grid = element_blank())
pdf("~/Nutstore Files/Tobin/Previous/0aa_inframe_histgram_insMut_addAll_v2.pdf", width = 5, height = 5)
print(ggpubr::ggarrange(p1, p2, nrow = 1, common.legend = T))
dev.off()
