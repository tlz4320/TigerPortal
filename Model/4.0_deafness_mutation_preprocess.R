dvd_mutation <- read.table("~/data/all_mutation_predict/DVD_frameshift_anno.hg38_multianno.txt", 
                           sep="\t", quote = '"', header = T )
table(dvd_mutation$avsnp150 == ".")
dvd_mutation <- dvd_mutation[dvd_mutation$avsnp150 != ".",]
dvd_mutation <- dvd_mutation[grep("exonic", dvd_mutation$Func.refGene),]
dvd_mutation <- dvd_mutation[dvd_mutation$Func.refGene != "ncRNA_exonic",]
dvd_mutation$AF <- as.numeric(dvd_mutation$gnomad312_AF)
dvd_mutation$AF[is.na(dvd_mutation$AF)] <- 0
dvd_mutation <- dvd_mutation[order(dvd_mutation$AF, decreasing = T),]
dvd_mutation <- dvd_mutation[grep("patho", str_to_lower(dvd_mutation$CLNSIG)),]
table(dvd_mutation$AF == 0)
...
seq_info <- seq_info[seq_info$Experiment.Number %in% total_used_seq$result,]
mut <- unique(seq_info$Locus)
mut_id <- lapply(mut, function(x){
  gene <- unlist(strsplit(x, "[-]"))
  m <- gene[2]
  m <- str_sub(m, 1, -3)
  gene <- str_to_upper(gene[1])
  tmp <- dvd_mutation[dvd_mutation$Gene.refGene == gene,]
  res <- tmp$avsnp150[grep(m, tmp$AAChange.refGene)]
  if(length(res) == 0){
    return("")
  }
  return(res)
})
which(unlist(lapply(mut_id, function(x){x[1] == ""})))
mut[which(unlist(lapply(mut_id, function(x){x[1] == ""})))]

mut_id[[3]] <- "rs397516304"
mut_id[[4]] <-  "rs111033347"
mut_id[[5]] <-  "rs397516320"
mut_id[[6]] <- "rs730880367"
mut_id[[15]] <- "rs199469706"

mut_id[[19]] <- "rs587776546"
mut_id[[21]] <- "rs80338903"
mut_id[[25]] <- "rs796051864"
mut_id[[27]] <- "rs869320674"
mut_id[[32]] <- "rs876657727"

which(unlist(lapply(mut_id, function(x){length(x) != 1})))
mut[which(unlist(lapply(mut_id, function(x){length(x) != 1})))]
mut_id[[10]] <- "rs796051861"
mut_id[[14]] <- "rs397517451"

mut[which(unlist(lapply(mut_id, function(x){!x %in% mutation_info$V6 & x != ""})))]
mut[which(unlist(lapply(mut_id, function(x){x[1] == ""})))]
unfound_mut <- list()
load("~/data/project/ear_project/gene_therapy_ll/largest_cds.rda")
unfound_mut[["USH1C-1220dG"]] <- generateCDS("USH1C", checkMut("c.1220dG"))
unfound_mut[["SANS-84iC"]] <- generateCDS("USH1G", checkMut("c.84iC"))
unfound_mut[["whirlin-643dG"]] <- generateCDS("WHRN", checkMut("c.643dG"))
unfound_mut[["TMC1-150dT"]] <- generateCDS("TMC1", checkMut("c.150dT"))
unfound_mut[["TMC1-766dT"]] <- generateCDS("TMC1", checkMut("c.766dT"))
unfound_mut[["OTOF-2166iC" ]] <- generateCDS("OTOF", checkMut("c.2166iC"))
largest_cds$Seq[largest_cds$V13 == "OTOF"]
library(parallel)
deaf_design <- list() 



for(index in 1 : length(mut)){
  mutid <- mut[[index]]
  if(mut_id[[index]] == ""){
    inputseq <- str_to_upper(unfound_mut[[mutid]]$seq)
  } else {
    inputseq <- mutation_info$V7[mutation_info$V6 == mut_id[[index]]]
    if(any(unlist(mutation_info[mutation_info$V6 == mut_id[[index]], 
                                c("V8","V9")]) != 0)){
      print(mut_id[[index]])
    }
  }
  ...
  ###如何是OTOF 1236delC取出和实验一致的位置
  if(mutid == "OTOF-1236dC"){
    coloredSeq <- coloredSeq[2,]
  }
  ...
  tmp_res[["mut"]] <- mutid
  deaf_design[[mutid]] <- tmp_res
} 
deaf_for_indelphi <- lapply(deaf_design, function(tmp){
  sgInfos <- tmp$infos
  mutid <- tmp$mut
...
deaf_for_indelphi <- data.frame(do.call(rbind, deaf_for_indelphi))
# write.table(deaf_for_indelphi,"~/data/all_mutation_predict/deaf_indelphi.csv",
# col.names = T, row.names = F,quote = T,sep = ",")

setwd("~/data/all_mutation_predict/deaf_result/")
...
infos_list <- list()
region_list <- list()
for(i in 1 : length(deaf_design)){
  tmp <- deaf_design[[i]]
  sgs_list[[tmp$mut]] <- tmp$infos
  infos_list[[tmp$mut]] <- tmp$infos2
  if(tmp$mut == "PCDH15-16dT"){
    region_list[[tmp$mut]] <- unlist(mutation_info[mutation_info$V6 == "rs397517451", 
                                                   c("V8","V9")])
  } else {
    region_list[[tmp$mut]] <- c(0,0)
  }

}

library(parallel)
deaf_for_indelphi$mut <- str_remove(deaf_for_indelphi$id, "-id_[0-9]*")
mutids <- unique(deaf_for_indelphi$mut)
tmp_total_result <- mclapply(mutids, function(mutid){
  sgs <- sgs_list[[mutid]]
  info <- infos_list[[mutid]]
  cds_region <- region_list[[mutid]]
  ids <- deaf_for_indelphi$id[deaf_for_indelphi$mut == mutid]
...
for(name in names(tmp_total_result)){
  deaf_result[[name]] <- tmp_total_result[[name]]
}
rm(tmp_total_result)
deaf_sgs <- lapply(deaf_result, function(x){
  x[['sgs']]
})
deaf_sgs <- data.frame(do.call(rbind, deaf_sgs))
...
merged_sgs <- data.frame(rbind(top100_sgs, deaf_sgs))
merged_sgs$shape <- "Top100"
merged_sgs$shape[merged_sgs$id %in% deaf_sgs$id] <- "DeafMut"

p <- ggplot(merged_sgs) + geom_point(aes(x = cut_mut_dis, y =diff, color = inframe, 
                                         fill = aa0_inframe, shape = shape), 
...
pdf("~/Nutstore Files/Tobin/Predict/volcano_plot_of_top100_and_deafmut.pdf", width = 9, height = 6)
print(p)
dev.off()

merged_result <- deaf_result
for(name in names(top100_result)){
  merged_result[[name]] <- top100_result[[name]]
}
aa0_result <- lapply(merged_result, function(x){
  x[['0AA']]
})
aa0_result <- data.frame(do.call(rbind, aa0_result))
inframe_result <- lapply(merged_result, function(x){
  x[["1AA"]]
})
inframe_result <- data.frame(do.call(rbind, inframe_result))
...
plot_data <- merge(plot_data, merged_sgs, by="id")
...
pdf("~/Nutstore Files/Tobin/Predict/Del1_top10_edit_LogoPlot_fixed2.pdf", width = 11, height = 9 / 13 * 6)
...
pdf("~/Nutstore Files/Tobin/Predict/Ins1_top10_edit_LogoPlot_fixed2.pdf", width = 11, height = 9 / 13 * 6)
draw(ht, annotation_legend_list = lgd_list)
tmp_top10_wt_sgs <- top10_wt_sgs[nrow(top10_wt_sgs) : 1,]
...
plot_data2 <- merge(mutgene, plot_data2, by.x="V6", by.y="mutid", all.y=T)
plot_data2$idDeaf <- plot_data2$id %in% deaf_sgs$id
plot_data2$addName <- paste(plot_data2[,2], plot_data2[,1], sep="-")
plot_data2$addName[plot_data2$shape == "Other"] <- ""
plot_data2$addName[plot_data2$idDeaf] <- plot_data2$id[plot_data2$idDeaf]
plot_data3 <- plot_data2[(plot_data2$addName != "" & plot_data2$inframe > 50 & plot_data2$Pct > 0.5) & !plot_data2$idDeaf,]
plot_data4 <- plot_data2[(plot_data2$addName != "" & plot_data2$inframe > 50 & plot_data2$Pct > 0.5) & plot_data2$idDeaf,]
p <- ggplot(plot_data, aes(x = Pct * 100, y = inframe)) + 
  ...
  geom_text_repel(data = plot_data3, 
                  ...
                  max.overlaps = 10000) + 
  geom_text_repel(data = plot_data4, 
                  aes(label = addName), min.segment.length = unit(0, 'lines'),
                  max.overlaps = 10000, color = "red") + 
  scale_color_gradientn(colours = c("blue",  "red"),limits = c(0,100),
                        name = "Inframe%") + theme_bw() + 
  ...
  theme_bw()
pdf("~/Nutstore Files/Tobin/Predict/WT_of_AA_IDchange_add_deaf.pdf", width = 7.5, height = 6)
print(p)
dev.off()
