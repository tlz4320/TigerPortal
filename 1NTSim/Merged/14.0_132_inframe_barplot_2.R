...
plot_data2$type <- factor(plot_data2$isInsert, levels = c("Insert", "Delete"), labels = c("IStrand", "CStrand"))
...
pdf("~/Nutstore Files/Tobin/Merged1NT/132_inframe_0aa_pointPlot_for_CStrand_gradient_alpha_v8.pdf", width = 5, height = 4)
ggplot() + 
...
pdf("~/Nutstore Files/Tobin/Merged1NT/132_inframe_0aa_CStrand_legend_v3.pdf", width = 3, height = 7)

legend_plot <- data.frame(id = 1 : 132, 
                          color = rev(alpha("#B46F28FF", sort(plot_data2$alpha[plot_data2$isInsert == "Insert"]))))
...
p2 <- ggplot(sgCmp_bstrand) + 
  ...
  theme_bw() + theme(axis.text.x = element_text(angle = 90, 
                                                ...
                                                vjust = 0.5), 
                     panel.grid  = element_blank())
pdf("~/Nutstore Files/Tobin/Merged1NT/132_inframe_0aa_barplot_for_CStrand_Ins1.pdf", width = 20, height = 10)
...
orderlist <- lapply(split(sgCmp_sel[,c("inframePct", "0AA")], sgCmp_sel$id), function(x){
  p1 <- pnorm(x[1,1],mean = x[2,1], sd = sd_b)
  p2 <- pnorm(x[1,2],mean = x[2,2], sd = sd_0aa_b)
  if(p1 < 0.5){
    p1 <- 2 * p1
  } else {
    p1 <- 2 * (1 - p1)
...
p2 <- ggplot(sgCmp_bstrand) + 
  ...
  theme_bw() + theme(axis.text.x = element_text(angle = 90, 
                                                ...
                                                vjust = 0.5), 
                     panel.grid  = element_blank())
pdf("~/Nutstore Files/Tobin/Merged1NT/132_inframe_0aa_barplot_for_CStrand_Ins1_reorder.pdf", width = 20, height = 10)
ggpubr::ggarrange(p1, p2, nrow = 2, common.legend = T)
dev.off()
openxlsx::write.xlsx(orderlist,file='~/Nutstore Files/Tobin/Merged1NT/132_inframe_0aa_barplot_for_CStrand_Ins1_reorder.xlsx', rowNames=F, colNames=T)
...
p1 <- ggplot(plot_data) + geom_bar(aes(x = x, y = counts),
  ...
  xlab("Inframe Frequency") + ylab("CStrand Counts") +
...
p2 <- ggplot(plot_data) + geom_bar(aes(x = x, y = counts),
  ...
  ylab("BStrand Counts") + xlab("") + 
  theme_bw() + 
  theme(axis.text.x = element_blank(), panel.grid = element_blank(),
        plot.margin =  unit(c(-1,0,1,0.2), "cm"))

pdf("~/Nutstore Files/Tobin/Merged1NT/132_inframe_histgram_for_Cstrand_Ins1.pdf", width = 6, height = 6)
...
pdf("~/Nutstore Files/Tobin/Merged1NT/132_0aa_inframe_histgram_for_Cstrand_Ins1.pdf", width = 6, height = 6)
ggpubr::ggarrange(p1, p2, ncol = 1, common.legend = T)
dev.off()
