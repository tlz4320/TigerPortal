...
output_indel_tissue <- plot_mat * 100
output_indel_tissue <- data.frame(output_indel_table[, 1 : 13])
colnames(output_indel_tissue) <- c(paste0("+", 5 : 1), paste0("-", 1 : 8))
output_indel_tissue$sgName <- total_pair_sg_cor$mutation
output_indel_tissue <- output_indel_tissue[,c(ncol(output_indel_tissue), 1 : (ncol(output_indel_tissue) -1))]
output_indel_cell <- plot_mat * 100
output_indel_cell <- data.frame(output_indel_cell[, ncol(output_indel_cell) :15 ])
colnames(output_indel_cell) <- c(paste0("+", 5 : 1), paste0("-", 1 : 8))
output_indel_cell$sgName <- total_pair_sg_cor$mutation
output_indel_cell <- output_indel_cell[,c(ncol(output_indel_cell), 1 : (ncol(output_indel_cell) -1))]

openxlsx::write.xlsx(list(Tissue = output_indel_tissue, 
                          N2a = output_indel_cell), "~/Nutstore Files/Tobin/sTable/s4Table.xlsx", rowNames=F, colNames=T)
...
inframe_result <- openxlsx::read.xlsx("~/Nutstore Files/Tobin/Previous/previous_tissue_inframe_in_indel.xlsx")
...
plot_mat_left <- plot_mat[,1 : (length(plot_region) - 1)]
top_anno <- HeatmapAnnotation(acc = anno_barplot(col_acc[1 : (length(plot_region) - 1)], 
                                                 border = T,
                                                 gp = gpar(fill = "#494490"), 
                                                 bar_width = 0.5,
                                                 width = unit(2, "cm")),
...
plot_mat_right <- plot_mat[,(length(plot_region) + 1) : ncol(plot_mat)]
top_anno2 <- HeatmapAnnotation(acc = anno_barplot(col_acc[(length(plot_region) + 1) : ncol(plot_mat)], 
                                                 border = T,
                                                 gp = gpar(fill = "#494490"), 
                                                 bar_width = 0.5,
                                                 width = unit(2, "cm")),
...
left_anno2 <- rowAnnotation(
  Cor = anno_barplot(total_pair_sg_cor$cor, 
                      border = T,
                      gp = gpar(fill = "red"), 
                      bar_width = 0.5,
                      axis_param = list(direction = "reverse", 
...
plot_mat3 <- matrix(0, ncol = length(plot_region) - 1, nrow = nrow(total_pair_sg_cor_indelphi))
...
top_anno3 <- HeatmapAnnotation(acc = anno_barplot(col_acc3, 
                                                 border = T,
                                                 gp = gpar(fill = "#494490"), 
                                                 bar_width = 0.5,
                                                 width = unit(2, "cm")),
...
plot_mat4 <- matrix(0, ncol = length(plot_region) - 1, nrow = nrow(total_pair_sg_cor_forecast))
...
top_anno4 <- HeatmapAnnotation(acc = anno_barplot(col_acc4, 
                                                 border = T,
                                                 gp = gpar(fill = "#494490"), 
                                                 bar_width = 0.5,
                                                 width = unit(2, "cm")),
                              pos4 = anno_empty(border = F, width = unit(2, "mm")))
...
pdf("~/Nutstore Files/Tobin/Merged1NT/118_pair_indel_table_v4.pdf", 
    width = 25, height = 44)
...
decorate_annotation("pos3",slice = 1, {
...
decorate_annotation("pos4",slice = 1, {
...
total_pair_sg_cor$group <- "Ours"
total_pair_sg_cor_forecast$group <- "ForeCast"
total_pair_sg_cor_indelphi$group <- "IndelPhi"
plot_data <- data.frame(rbind(total_pair_sg_cor[,-7], total_pair_sg_cor_forecast, total_pair_sg_cor_indelphi))

library(ggbeeswarm)
# pdf("~/Nutstore Files/Tobin/Merged1NT/Indel1_pct_132.pdf", width = 8,height = 6)
ggplot(plot_data) + geom_boxplot(aes(x = group, y = cor), outlier.shape = NA) + 
  geom_quasirandom(aes(x = group, y = cor)) + 
