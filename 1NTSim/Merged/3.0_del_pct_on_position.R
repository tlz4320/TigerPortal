...
###到这里只剩下282个样本了
# save(seqs_table, select_edit_pattern, file="~/data/project/ear_project/gene_therapy_ll/Result/first_second_del_table_data.rda")
...
pdf("~/data/project/ear_project/gene_therapy_ll/Result/merged_Del1_seq_logo_final_v3.pdf", 
    width = 6, height = 80)
...
pdf("~/data/project/ear_project/gene_therapy_ll/Result/merged_Del1_seq_logo_+-1.pdf", 
    width = 6, height = 25)
Heatmap(plot_mat_sel2, 
        name = "del1 Pct",
        column_split = NULL,
        column_title=NULL,
        row_title = NULL,
...
pdf("~/data/project/ear_project/gene_therapy_ll/Result/merged_Del1_seq_logo_12_sep.pdf", 
    width = 6, height = 20)
Heatmap(plot_mat_sel2, 
        name = "del1 Pct",
        column_split = NULL,
...
pdf("~/data/project/ear_project/gene_therapy_ll/Result/merged_Del1_seq_logo_5678_sep.pdf", 
    width = 6, height = 25)
Heatmap(plot_mat_sel2, 
        name = "del1 Pct",
        column_split = NULL,
...
sg_info_sel <- sg_info[!sg_info$lowCount,]
#去掉低质量的序列
rm_id <- sg_info_sel$pair[sg_info_sel$id %in% c("Sg_21_144","Sg_6_37","Sg_6_38","Sg_7_45","Sg_17_115")]
sg_info_sel <- sg_info_sel[!sg_info_sel$pair %in% rm_id,]
...
del1_besides_2nt_stat <- besides_nt_stat
save(del1_besides_2nt_stat, file="~/Nutstore Files/Tobin/Merged1NT/del1_beside_2nt.rda")
...
openxlsx::write.xlsx(output_table, 
                     file="~/Nutstore Files/Tobin/Merged1NT/merged_del1_left_right_2nt_pie_data.xlsx", 
...
ht <- Heatmap(fake_plot_mat, 
              ...
              cell_fun = function(j, i, x, y, w, h, fill) {
                ...
                if(!is.null(pct) ){
                  ...
                  facy <- as.numeric(h) / maxy * 0.4
                  facx <- as.numeric(w) / maxx * 0.4
                  for (i in 1L:nx) {
                    n <- max(10, floor(200 * dx[i]))
                    P <- t2xy(seq.int(pct[i], pct[i + 1], length.out = n))
...
pdf("~/Nutstore Files/Tobin/Merged1NT/merged_left_right_nt_pie_chart_del_v5.pdf", 
    width = 16, height = 15)
...
output_table <- output_table[,c(6, 5, 1,2,3,4)]
openxlsx::write.xlsx(output_table, 
                     file="~/Nutstore Files/Tobin/Merged1NT/merged_del1_left_right_nt_pie_data.xlsx", 
                     rowNames=F, colNames=T)
...
pdf("~/Nutstore Files/Tobin/Merged1NT/merged_del1_left_right_nt_pie.pdf", width = 6, height = 5)
draw(ht, annotation_legend_list = lgd_list)
dev.off()
