...
lr_del_nt_stat <- openxlsx::read.xlsx("~/Nutstore Files/Tobin/Merged1NT/merged_del1_left_right_nt_pie_data.xlsx")
...
for(id in unique(sel_34same_sample$id)){
  ...
  if(all(sgs %in% sample_info$sg)){
    if(unique(sample_info$sameContext[sample_info$sg == sgs]) != "Same"){
      tmp_pred <- lr_del_nt_stat[lr_del_nt_stat$leftNT == del_one_left & 
                                   lr_del_nt_stat$rightNT == del_one_right, c(-1, -2)]
      tmp_pred <- unlist(tmp_pred)
      del_one_pct <- tmp_pred[del_one_left]
    }
  }
  ...
  total_plot_data[[id]] <- data.frame(Pct = c(ins_one_pct, del_one_pct), Pct2 = c(100 - ins_one_pct, 
                                      ...
                                      sg = sgs, NT = c(ins_one_left, del_one_left),  rightNT = c(ins_one_right, del_one_right))
}
...
grDevices::cairo_pdf("~/Nutstore Files/Tobin/Merged1NT/merged_Context_oneNT_Del1_point_link_plot_changeColor_v3_add_pred_bulge_cause.pdf",
                     width = 6, height = 4)
...
lr_del_2nt_stat <- openxlsx::read.xlsx("~/Nutstore Files/Tobin/Merged1NT/merged_del1_left_right_2nt_pie_data.xlsx")
total_plot_data <- list()
for(id in unique(sel_34same_sample$id)){
  sgs <- sel_34same_sample$sg[sel_34same_sample$id == id]
  ins_one_seq <- wt_seqs[[sgs[1]]]
  del_one_seq <- wt_seqs[[sgs[2]]]
  ins_one_left <- paste0(ins_one_seq[c(19,20)], collapse = "")
  del_one_left <- paste0(del_one_seq[c(19,20)], collapse = "")
  del_one_left_1nt <- del_one_seq[20]
  ins_one_right <- paste0(ins_one_seq[c(21,22)], collapse = "")
  del_one_right <- paste0(del_one_seq[c(21,22)], collapse = "")
  ins_one_stat <- remain_del1_pct_table[[sgs[1]]]
  del_one_stat <- remain_del1_pct_table[[sgs[2]]]
  ...
  if(all(sgs %in% sample_info$sg)){
    if(unique(sample_info$sameContext[sample_info$sg == sgs]) != "Same"){
      tmp_pred <- lr_del_2nt_stat[lr_del_2nt_stat$leftNT == del_one_left & 
                                    lr_del_2nt_stat$rightNT == del_one_right, c(-1, -2)]
      tmp_pred <- unlist(tmp_pred)
      del_one_pct <- tmp_pred[del_one_left_1nt]
    }
  }
...
grDevices::cairo_pdf("~/Nutstore Files/Tobin/Merged1NT/merged_Context_oneNT_Del1_point_link_plot_changeColor_v3_add_pred2nt_bulge_cause.pdf",
                     width = 6, height = 4)
ggplot(total_plot_data_sel2, aes(x = type, y = Pct, color = sameContext, group = id)) +
  geom_line() +
  geom_point(aes(fill = deleteLeft),
