...
sg_anno2 <- data.frame(sg = c("Sg_9_63",
                              ...
                              "Sg_24_184"), 
                       dis = 1)
sg_anno3 <- data.frame(sg = c("Sg_21_145",
                              "Sg_12_81",
                              "Sg_16_107",
                              "Sg_15_105",
                              "Sg_10_65",
                              ...
                              "Sg_20_135",
                              "Sg_23_155"), 
                       dis = rep(c(0,0,0,0,0,1,1,1,1), rep(2, 9)))
...
sg_anno6 <- data.frame(sg = c("Sg_20_136",
                              "Sg_23_156",
                              "Sg_24_230",
                              "Sg_24_229",
                              "Sg_24_231",
...
sg_anno7 <- data.frame(sg = c("Sg_11_77",
                              "Sg_8_56",
                              "Sg_24_276",
                              "Sg_24_275",
                              "Sg_1_2",
                              ...
                              "Sg_24_218"), 
                       dis = 1)
sg_anno8 <- data.frame(sg = c("Sg_1_6",
                              "Sg_2_14",
                              "Sg_3_17",
                              "Sg_1_3",
                              "Sg_19_132",
...
sg_dis_anno <- data.frame(do.call(rbind, list(sg_anno2,
                                              sg_anno3, sg_anno4,
                                              sg_anno5)))
sg_ggg_anno <- data.frame(do.call(rbind, list(sg_anno6, sg_anno7, sg_anno8)))
...
for(id in unique(sample_info$id)){
  ...
  if(ins_one_left != del_one_left){
    ...
    tmp_diff <- ins_one_diff$diff
    names(tmp_diff) <- ins_one_diff$NT
...
# total_plot_data2 <- total_plot_data2[total_plot_data2$sameLeft,]
pdf("~/Nutstore Files/Tobin/Merged1NT/ins1_link_plot_no_predict_v3.pdf", width = 8, height = 6)

ggplot(total_plot_data2, aes(x = type, 
                             y = diff, 
                             color = NT, 
                             fill = NT, 
                             group = id)) +
  geom_line(aes(x = type, y = diff, linetype = ltp)) + 
  geom_point(color = total_plot_data2$color,
             size = 3, stroke = 1,
             shape = total_plot_data2$shape) + xlab("") + ylab("Normalized(%)") +
...
total_plot_data3$diff2 <- total_plot_data3$diff - total_plot_data3$dis
pdf("~/Nutstore Files/Tobin/Merged1NT/ins1_link_plot_no_predict_v2.pdf", width = 8, height = 6)

ggplot(total_plot_data3, aes(x = type, y = diff2, color = NT, fill = NT, group = id)) +
  geom_line(aes(x = type, y = diff2, linetype = ltp)) + 
  geom_point(size = 2, 
             shape = total_plot_data3$shape, 
             color ="black") + xlab("") + ylab("Normalized Similarity") +
  facet_wrap(~dis, nrow = 1) +
  scale_fill_manual(values = c("A" = "#2F89FC",
                               "T" = "#30E3CA",
                               "C" = "#66CD00",
...
##写出输入给indelphi用
for_indelphi <- sgCmp#[sgCmp$id2 %in% total_plot_data2$sg,]
for_indelphi$cutpos <- unlist(lapply(1 : nrow(for_indelphi), function(i){
  sg <- paste0(for_indelphi$sgRNA[i],for_indelphi$sgRNA_NGG[i])
...
ToNX::write_tb(forcast_file, file="~/132_for_forecast.txt", col.names = F)
...
total_plot_data3$type <- factor(total_plot_data3$type, levels = c("Ref", "Target", "inDelphi", "FORECasT"))
...
total_plot_data4 <- total_plot_data3[total_plot_data3$sameLeft,]
pdf("~/Nutstore Files/Tobin/Merged1NT/ins1_link_plot_add_predict_v2.pdf", width = 8, height = 6)
ggplot(total_plot_data4, aes(x = type, y = diff, color = NT, fill = NT, group = id)) +
  geom_line(aes(x = type, y = diff, linetype = ltp)) + 
  geom_point(size = 2, 
             shape = total_plot_data4$shape, 
             color ="black") + xlab("") + ylab("Normalized(%)") +
  facet_wrap(~dis, nrow = 1) +
  scale_fill_manual(values = c("A" = "#2F89FC",
                               "T" = "#30E3CA",
  ...
  theme_classic2() + theme(axis.text.x = element_text(angle = 45, hjust = 1))
dev.off()
wilcox.test(diff ~ type, total_plot_data4[total_plot_data4$type %in% c("Ref", "Target"),])
wilcox.test(diff ~ type, total_plot_data4[total_plot_data4$type %in% c("inDelphi", "Target"),])
wilcox.test(diff ~ type, total_plot_data4[total_plot_data4$type %in% c("FORECasT", "Target"),])
