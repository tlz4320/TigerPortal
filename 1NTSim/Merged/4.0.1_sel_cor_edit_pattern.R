...
for(pairids in sel_pair$pair){
  ...
  ####画柱状图
  p_l1 <- lapply(names(total_indel_table_first_second_rev[[ids[1]]]), function(n){
    region <- c(-18: 12)
    sel_sg <- total_indel_table_first_second_rev[[ids[1]]][[n]]
  ...
  p_l2 <- lapply(names(total_indel_table_first_second_rev[[ids[2]]]), function(n){
    region <- c(-18: 12)
    sel_sg <- total_indel_table_first_second_rev[[ids[2]]][[n]]
    long_del <- sum(sel_sg[sel_sg[,1] < min(region), 2])
    long_ins <- sum(sel_sg[sel_sg[,1] > max(region), 2])
    long_res <- data.frame(indel_size = c("<=-19", ">=13"), fq = c(long_del, long_ins))
    tmp_region <- region[!region %in% sel_sg[,1]]
  ...
  merge_plot2 <- annotate_figure(merge_plot2, top = text_grob(paste0(pairids, "  Cor: ", round(sel_pair$cor[sel_pair$pair == pairids], 3)), 
                                                              color = "black", face = "bold", size = 14))
  pdf(paste0("~/data/project/ear_project/gene_therapy_ll/Result/sel_edit_pattern_version2_", pairids,".pdf"), width = 6, height = 4)
  print(merge_plot2)
  dev.off()
}
dev.off()
