...
# pdf("~/Nutstore Files/Tobin/Merged1NT/del1_inframe_hist.pdf", width = 6, height = 4)
...
# openxlsx::write.xlsx(sg_info_sel, file="~/Nutstore Files/Tobin/Merged1NT/Del1_inframe_ratio_info.xlsx", rowNames=F, colNames=T)
...
# save(inframe_result_processed, file="~/Nutstore Files/Tobin/Merged1NT/del1_inframe_spec_result.rda")
...
# pdf("~/Nutstore Files/Tobin/Merged1NT/del1_inframe_aa_change.pdf", width = 6, height = 12)
# ggplot(plot_data) + 
...
# openxlsx::write.xlsx(sg_info_sel, file="~/Nutstore Files/Tobin/Merged1NT/Del1_inframe_ratio_info_add_predict.xlsx", rowNames=F, colNames=T)
sg_info_sel <- openxlsx::read.xlsx("~/Nutstore Files/Tobin/Merged1NT/Del1_inframe_ratio_info_add_predict.xlsx")
sg_info_sel_noBulge <- sg_info_sel[seq(2, nrow(sg_info_sel), 2),]
sg_info_sel_withBulge <- sg_info_sel[seq(1, nrow(sg_info_sel), 2),]
sg_info_sel_output <- sg_info_sel_withBulge
sg_info_sel_output$inframe_reference <- sg_info_sel_noBulge$inframePct
sg_info_sel_output$inframeSe_reference <- sg_info_sel_noBulge$inframeSe
sg_info_sel_output <- sg_info_sel_output[,-4]
colnames(sg_info_sel_output)[c(5,6)] <- c("inframe_target", "inframeSe_target")
openxlsx::write.xlsx(sg_info_sel_output, file="~/Nutstore Files/Tobin/Merged1NT/Del1_inframe_ratio_info_add_predict_target.xlsx", rowNames=F, colNames=T)
...
plot_data_0aa$type <- factor(plot_data_0aa$type, levels = c("Bulge","noBulge",  "IndelPhi", "forecast"), 
                             labels = c("target", "reference", "InDelphi", "FOREcasT"))
pdf("~/Nutstore Files/Tobin/Merged1NT/del1_0AA_violine_of_inframe_all.pdf", width = 6, height = 4)
ggplot(plot_data_0aa, aes(x= type, y = pct2)) + 
  geom_violin(aes(fill = type)) + 
  geom_point(data = plot_data_0aa, aes(x= type, y = pct2), shape= 21,
...
# plot_data_0aa$color <- "gray"
# plot_data_0aa$color[plot_data_0aa$pct2 > 30] <- "#FF4500"
# plot_data_0aa_sel <- plot_data_0aa[plot_data_0aa$pct2 > 30,]
# library(rstatix)
# stat <- plot_data_0aa_sel %>%
#   wilcox_test(pct2 ~ type)
...
plot_data_0aa$reference30 <- plot_data_0aa$id %in% sg_info_sel$id[which(sg_info_sel$id %in% inframe30_noBulge$id) - 1]
plot_data_0aa$target30 <- plot_data_0aa$id%in% inframe30_withBulge$id
...
pdf("~/Nutstore Files/Tobin/Merged1NT/del1_0AA_violine_of_inframe_all3.pdf", width = 8, height = 5)
ggplot(plot_data_0aa, aes(x= type, y = pct2)) + 
...
openxlsx::write.xlsx(export_info_tmp, file="~/Nutstore Files/Tobin/Merged1NT/del1_inframe_0aa_plot_data.xlsx", rowNames=F, colNames=T)

pdf("~/Nutstore Files/Tobin/Merged1NT/del1_0AA_violine_of_inframe_large30.pdf", width = 6, height = 4)
...
dev.off()
plot_data_0aa_sel2 <- plot_data_0aa_sel[plot_data_0aa_sel$large30 == "Both>30",]
pdf("~/Nutstore Files/Tobin/Merged1NT/del1_0AA_violine_of_inframe_Both_30.pdf", width = 6, height = 4)
...
pdf("~/Nutstore Files/Tobin/Merged1NT/del1_0AA_violine_v2.pdf", width = 6, height = 4)
