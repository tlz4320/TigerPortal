...
total_pair_sg_cor <- total_pair_sg_cor[total_pair_sg_cor$id %in% names(sgRNA_pair_remain),]
total_pair_sg_cor <- total_pair_sg_cor[order(total_pair_sg_cor$pos, total_pair_sg_cor$cor),]
pdf("Result/all_pair_sample_result_peak_indel1.pdf", width = 10, height = 8)
...
total_edit_pattern <- lapply(c("A", "B", "C"), function(x){
  tmp <- list()
  for(name in names(indel1_sample)){
    if(x %in% names(total_data[[name]])){
      tmp[[name]] <- total_data[[name]][[x]]
    }
  }
  tmp
})
names(total_edit_pattern) <- c("A", "B", "C")
total_sg_cor_each_sample <- list()

for(name in names(total_edit_pattern)){
  edit_data <- total_edit_pattern[[name]]
  sgRNA_pair_remain <- sgRNA_pair[unlist(lapply(sgRNA_pair, function(x){
    x <- unlist(x)
...
pdf("~/data/project/ear_project/gene_therapy_ll/Result/sgRNA_pair_edit_pattern_each_sample_cor_indel1.pdf", width = 30, height = 6)
...
plot_data <- plot_data[plot_data$pair != "Sg_9_60-Sg_15_103",]
order_cor <- lapply(split(plot_data$cor, plot_data$pair), mean)
order_cor <- data.frame(pair = names(order_cor), cor = unlist(order_cor))
order_cor <- order_cor[order(order_cor$cor, decreasing = T),]
plot_data$pair <- factor(plot_data$pair, levels = order_cor$pair)
pdf("~/data/project/ear_project/gene_therapy_ll/Result/sgRNA_pair_edit_pattern_each_sample_cor_indel1_rmbad_reorder.pdf", width = 20, height = 6)
ggplot(plot_data, aes(x = pair, y = cor)) + 
  geom_boxplot() +
  geom_point(aes(color = sample), position = position_dodge2(width = 0.2)) + 
  geom_hline(yintercept = 0.8) + xlab("") + 
  theme_bw() + 
...
# plot_data$last <- ifelse(plot_data$pair %in% sample_order$pair[sample_order$test == "First"], "First", "Last")
# pdf("~/data/project/ear_project/gene_therapy_ll/Result/sgRNA_pair_edit_pattern_each_sample_cor_indel1_rmbad_add_check_color.pdf", width = 30, height = 6)
# ggplot(plot_data, aes(x = pair, y = cor)) + 
#   geom_boxplot() +
#   geom_point(aes(color = last), position = position_dodge2(width = 0.2)) + 
#   geom_hline(yintercept = 0.8) + 
#   theme_bw() + 
#   theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), 
#         text = element_text(size=15))
...
total_edit_pattern <- lapply(c("A", "B", "C"), function(x){
  tmp <- list()
  for(name in unlist(sgRNA_pair_remain)){
    if(x %in% names(total_data[[name]])){
      tmp[[name]] <- total_data[[name]][[x]]
    }
  }
