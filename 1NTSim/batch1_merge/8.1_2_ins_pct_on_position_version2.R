...
####最终要去掉一些低质量的结果
###首先判断Ins Reads是不是小于200条

sg_info$lowCount <- unlist(lapply(sg_info$id, function(id){
  tmp <- remain_ins1_pct_table[[id]]
  tmp <- tmp[tmp$pos %in% c(20:21),]
  tmp <- lapply(split(tmp$counts, tmp$pos), sum)
  tmp <- data.frame(pos = names(tmp), counts = unlist(tmp))
  all(tmp$counts < 200)
}))
...
sg_info_sel <- sg_info[!sg_info$lowCount & !sg_info$lowPct,]
#去掉低质量的序列
...
pdf("~/data/project/ear_project/gene_therapy_ll/Result/Ins1_seq_logo_final_v2.pdf", width =7, height = 40)
...
pdf("~/data/project/ear_project/gene_therapy_ll/Result/left_right_nt_pie_chart_v2_groupPct.pdf", width = 6, height = 5)
...
rownames(fake_plot_mat) <- c("A", "T", "C", "G")
colnames(fake_plot_mat) <- c("A", "T", "C", "G")
pdf("~/data/project/ear_project/gene_therapy_ll/Result/left_right_nt_pie_chart_v2_count.pdf", width = 6, height = 5)
ht <- Heatmap(fake_plot_mat, 
              name = "Name",
              column_split = NULL,
              column_title=NULL,
